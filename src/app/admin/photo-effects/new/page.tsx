...
export const metadata = {
  title: "New Photo Effect — Admin — Raxim",
};

export default async function NewPhotoEffectPage() {
  async function create(formData: FormData) {
    ...
    redirect(`/admin/photo-effects/${product.id}/edit`);
  }

  return (
    <div>
      <h1 className="text-3xl font-bold text-ink tracking-tight">New photo effect</h1>
      <Card className="mt-8 p-6 max-w-2xl">
        <form action={create} className="space-y-6">
          ...
          <Button type="submit">Create photo effect</Button>
        </form>
      </Card>
    </div>
  );
